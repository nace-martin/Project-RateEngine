        ...
        for line in import_or_export_lines:
            ...
            if code not in consolidated:
                consolidated[code] = {
                    ...
                    'exchange_rate': getattr(line, 'exchange_rate', None),
                    'base_exchange_rate': getattr(line, 'base_exchange_rate', None),
                    'caf_percent': getattr(line, 'caf_percent', None),
                    'provider_name': getattr(line, 'provider_name', None),
    ...
    def _calculate_spot_lines(self) -> List[CalculatedChargeLine]:
        ...
        for charge in ordered_charges:
            ...
            lines.append(CalculatedChargeLine(
                ...
                exchange_rate=fx_buy if charge.currency != 'PGK' else (output_fx_sell if output_currency != 'PGK' else None),
                base_exchange_rate=fx_buy if charge.currency != 'PGK' else (output_fx_sell if output_currency != 'PGK' else None),
                caf_percent=caf_pct,
                provider_name='SPOT Agent',
