...
class Command(BaseCommand):
    ...
    def _seed_cogs(self, code, org, dest, agent, curr,
        ...
        """
        Seeds Import COGS or LocalCOGSRate depending on the ProductCode category.
        """
        try:
        ...
        if is_local_rate_category(pc.category):
            # Local category -> seed in LocalCOGSRate
            if percent_rate:
                rate_type = 'PERCENT'
                amount = Decimal(percent_rate)
            elif per_kg:
                rate_type = 'PER_KG'
                amount = Decimal(per_kg)
            else:
                rate_type = 'FIXED'
                amount = Decimal(flat) if flat else Decimal('0.00')
            ...
            from pricing_v4.models import LocalCOGSRate
            matches = LocalCOGSRate.objects.filter(**lookup).order_by('id')
            if matches.exists():
                if matches.count() > 1:
                    self.stdout.write(
                        self.style.WARNING(
                            f"  ! Duplicate LocalCOGSRate rows found for {code} at {org} ({curr}); updating oldest match only"
                        )
                    )
                obj = matches.first()
                ...
                PricingDomainService.save_rate(LocalCOGSRate(**lookup, **seed_values))
            self.stdout.write(f"  - Seeded LocalCOGSRate {code} at {org} ({curr})")

        else:
            # Lane category -> seed in ImportCOGS
            seed_values = {
            ...
            self.stdout.write(f"  - Seeded ImportCOGS {code} {org}->{dest} ({curr})")
