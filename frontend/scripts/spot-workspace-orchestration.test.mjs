...
assert.ok(hookSrc.includes("inferProductCodeDomainFromShipmentContext"), "ProductCode requests must derive domain from trusted shipment context");
...
assert.ok(/target_id:\s*chargeId,\s*domain:\s*productCodeDomain/.test(hookSrc), "ProductCode requests must carry domain at decision level for schema compatibility");
assert.ok(hookSrc.includes("details: {") && hookSrc.includes("domain: productCodeDomain,"), "ProductCode requests must carry domain inside details for backend request creation");
